...
/**
 * Font Size Provider - Isolated plugin functionality
 *
 * Allows users to increase, decrease, and manually set font sizes
 * for selected text using DOM manipulation
 */
...
export const FontSizeProvider: React.FC<FontSizeProviderProps> = ({ children }) => {
  React.useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
      (window as any).registerEditorCommand?.('decreaseFontSize', decreaseFontSizeCommand);
      (window as any).registerEditorCommand?.('increaseFontSize', increaseFontSizeCommand);
      (window as any).registerEditorCommand?.('setFontSize', setFontSizeCommand);
    }
  }, []);
...
/**
 * Font Size Commands - Restored from FontSizePlugin
 * Uses DOM manipulation for font size application
 */

// Decrease font size command
const decreaseFontSizeCommand = () => {
  applyFontSizeChange(-2);
  updateFontSizeInput();
};

// Increase font size command
const increaseFontSizeCommand = () => {
  applyFontSizeChange(2);
  updateFontSizeInput();
};

// Set font size command (for manual input)
const setFontSizeCommand = (size: any) => {
  if (!size) return;
